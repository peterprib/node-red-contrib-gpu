function columns(a,columns,pipeline=false,blocks=1) {
	const details=this.getDetailsColumns(a,columns);
	if(this.logger.active) this.logger.send({label:"loadColumns",details:details});
	if(blocks==1) {
...
function rows(a,rows,pipeline=false,blocks=1) {
	const details=this.getDetailsRows(a,rows);
	if(this.logger.active) this.logger.send({label:"loadRows",details:details});
	if(blocks==1) {
